...
Litex是一门简单直观、易学易用的形式化语言。Litex的愿景是每一步逻辑推理、每一段关键代码、每一个 AI Agent 都能被形式化验证所守护并加速。这里我们举例说明，相比主流形式化语言如Lean，Litex把构建形式化数据集的成本和门槛降低了10倍。您可以在官网沙盒[1]上运行这里的例子和更多例子。复旦数学博二在读的沈嘉辰是Litex发明人，洪昭宣是Litex工具的开发者。
