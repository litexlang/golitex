...
type StoredUniSpecFact struct {
	IsTrue     bool
	FuncParams *[]st.Fc // 和存在Fact里的FuncFact共享slice，只要是共享，那我就用*[]，虽然确实 Fact里的 FuncFact 日后不会改变，且二者再也不相见了
	Fact       *st.UniFactStmt
}
...
type EqualFactMemoryTreeNode struct {
	...
	Values *[]st.Fc // VERY IMPORTANT: THIS IS PTR TO SLICE, NOT SLICE, Because every owner of this piece of memory can modify it, and this modification is shared between owners (every owner can see this modification).
...
type StoredPropMemDictNode struct{ Def *st.DefConPropStmt }
...
type StoredObjMemDictNode struct{ Def *st.DefObjStmt }
type ObjMem struct {
...
type StoredFnMemDictNode struct{ Def *st.DefConFnStmt }
