...
func (exec *Executor) verifyCondFact(stmt *parser.ConditionalFactStmt) error {
	// TODO : If there are symbols inside prop list that have  equals,we loop over all the possible equivalent situations and verify literally
...
func (exec *Executor) verifyCondFactLiterally(stmt *parser.ConditionalFactStmt) error {
	exec.newEnv()
