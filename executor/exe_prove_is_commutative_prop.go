...
func (exec *Executor) proveIsCommutativePropStmt(stmt *ast.ProveIsCommutativePropStmt) ast.StmtRet{
	...
	if !ok {
		return ast.StmtErrRet(stmt, fmt.Sprintf("prop %s is not defined", stmt.SpecFact.PropName))
	}

	def := definedStuff.Defined

	if len(def.DefHeader.Params) != 2 {
		return ast.StmtErrRet(stmt, fmt.Sprintf("prop %s has %d params, but 2 params are expected", stmt.SpecFact.PropName, len(def.DefHeader.Params)))
	}
	...
	for _, param := range stmt.SpecFact.Params {
		...
		if !ok {
			return ast.StmtErrRet(stmt, fmt.Sprintf("param %s is not an atom", param))
		}
...
func (exec *Executor) proveIsCommutativePropStmtBody(proofs []ast.Stmt, fact ast.SpecificFactStmt, rightToLeft ast.SpecificFactStmt) (ast.StmtRet, []ast.VerRet, []ast.StmtRet) {
	...
	if ret.IsErr() {
		return ast.StmtErrRet(fact, ret.String()), nil, nil
	}
	...
	if state.IsUnknown() {
		return ast.StmtErrRet(rightToLeft, fmt.Sprintf("proof in %s must be proved to be true, but %s is not true", glob.KeywordComProp, rightToLeft)), nil, nil
	}
	verifyProcessMsgs = append(verifyProcessMsgs, state.GetVerifyProcess()...)

	return ast.NewTrueStmtEmptyRet(rightToLeft), verifyProcessMsgs, innerStmtRets
}
