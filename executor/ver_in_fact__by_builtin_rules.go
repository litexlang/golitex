...
func (ver *Verifier) trueInFactBuiltinRules(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
	if len(stmt.Params) != 2 {
		return glob.ErrRet(fmt.Sprintf("invalid number of parameters for in fact: %d", len(stmt.Params)))
	}

	var verRet *glob.StmtRet = glob.NewEmptyStmtUnknown()
...
func (ver *Verifier) verInFactByLeftIsCartSetAndRightIsKeywordNonemptySet(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
...
func (ver *Verifier) falseInFactBuiltinRules(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
	// 任何东西不在空集里
...
func (ver *Verifier) nothingIsInEmptySet(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
...
func (ver *Verifier) verInFactByRightParamIsSetProduct(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
...
func (ver *Verifier) verInCartSet_DimAndElements(obj ast.Obj, cartSet *ast.FnObj, objCartSet *ast.FnObj, state *VerState) *glob.StmtRet {
	...
	return glob.NewStmtTrueWithVerifyProcess(glob.NewVerMsg(glob.StmtRetTypeTrue, "", 0, []string{msg}))
}

// verInFactByRightParamIsCartSet verifies a $in cart(...) by checking:
// 1. dim(a) = dim(cart(...))
// 2. a[i] $in cart(...).Params[i-1] for each i
func (ver *Verifier) verInFactByRightParamIsCartSet(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
	// Check if right side is cart(...)
	cartSet, ok := stmt.Params[1].(*ast.FnObj)
	if !ok {
		return glob.NewEmptyStmtUnknown()
...
func (ver *Verifier) ver_In_FnFcFn_FnTT(left ast.Obj, fnFcFn *ast.FnObj, state *VerState) *glob.StmtRet {
	ver.newEnv()
	defer ver.deleteEnv()
...
func (ver *Verifier) verInFactByLeftParamIsReturnValueOfUserDefinedFn(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
	fcFn, ok := stmt.Params[0].(*ast.FnObj)
	if !ok {
		return glob.NewEmptyStmtUnknown()
	}
...
func (ver *Verifier) litNumNotInNaturalByLiteralShape(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
...
func (ver *Verifier) litNumNotInIntegerByLiteralShape(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
...
func (ver *Verifier) litNumNotInNPosByLiteralShape(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
...
func (ver *Verifier) verInFactByLeftIsIndexOfObjInSomeSet(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
	left := stmt.Params[0]
...
func (ver *Verifier) verInFactByRightIsSetBuilder(stmt *ast.SpecFactStmt, state *VerState) *glob.StmtRet {
	...
	return glob.NewStmtTrueWithVerifyProcess(glob.NewVerMsg(glob.StmtRetTypeTrue, stmt.String(), stmt.Line, []string{"definition of set builder"}))
}
	...
	for _, item := range listSetFnObj.Params {
		...
		if verRet.IsTrue() {
			...
			if stmt.Params[0].String() == item.String() {
				return glob.NewStmtTrueWithVerifyProcess(glob.NewVerMsg(glob.StmtRetTypeTrue, stmt.String(), stmt.Line, []string{fmt.Sprintf("%s $in %s, %s = %s", stmt.Params[0], listSetFnObj.String(), stmt.Params[1], listSetFnObj)}))
			}

			return glob.NewStmtTrueWithVerifyProcess(glob.NewVerMsg(glob.StmtRetTypeTrue, stmt.String(), stmt.Line, []string{fmt.Sprintf("%s $in %s, %s = %s, %s = %s", stmt.Params[0], listSetFnObj.String(), stmt.Params[1], listSetFnObj, item, stmt.Params[0])}))
		}
	}
