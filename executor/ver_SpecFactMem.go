...
func (ver *Verifier) iterate_KnownSpecInLogic_InUni_applyMatch_new(stmt *ast.SpecFactStmt, knownFacts []env.SpecFact_InLogicExpr_InUniFact, state *VerState) *glob.VerRet {
	for i := len(knownFacts) - 1; i >= 0; i-- {
		...
		if err != nil {
			return glob.NewVerMsg(glob.StmtRetTypeUnknown, stmt.String(), stmt.GetLine(), []string{err.Error()})
		}
		if !ok {
			continue
		}
		...
		if verRet.IsErr() {
			return glob.NewVerMsg(glob.StmtRetTypeUnknown, stmt.String(), stmt.GetLine(), []string{verRet.String()})
		}
		if verRet.IsUnknown() {
			continue
		}
		...
		if !ok {
			return glob.NewVerMsg(glob.StmtRetTypeUnknown, stmt.String(), stmt.GetLine(), []string{"instantiatedLogicExpr is not a KnownSpecFact_InLogicExpr"})
		}
		...
		if verRet.IsTrue() {
			if state.WithMsg {
				return glob.NewVerMsg(glob.StmtRetTypeTrue, stmt.String(), stmt.GetLine(), []string{knownFactUnderLogicExpr.UniFact.String()})
			}
...
func (ver *Verifier) SpecFactSpecUnderLogicalExpr(knownFact *env.KnownSpecFact_InLogicExpr, stmt *ast.SpecFactStmt, state *VerState) *glob.VerRet {
	...
	if state.WithMsg {
		return glob.NewVerMsg(glob.StmtRetTypeTrue, stmt.String(), stmt.GetLine(), []string{knownFact.LogicExpr.String()})
	}
...
func (ver *Verifier) specFact_LogicMem(curEnv *env.EnvMemory, stmt *ast.SpecFactStmt, state *VerState) *glob.VerRet {
	...
	if got {
		for _, knownFact := range knownFacts {
			...
			if verRet.IsTrue() {
				if state.WithMsg {
					return glob.NewVerMsg(glob.StmtRetTypeTrue, stmt.String(), stmt.GetLine(), verRet.VerifyMsgs)
				}
				return verRet
			}
		}
...
func (ver *Verifier) iterate_KnownSpecInUniFacts_applyMatch_new(stmt *ast.SpecFactStmt, knownFacts []env.KnownSpecFact_InUniFact, state *VerState) *glob.VerRet {
	for i := len(knownFacts) - 1; i >= 0; i-- {
		...
		if verRet.IsTrue() {
			if state.WithMsg {
				return glob.NewVerMsg(glob.StmtRetTypeTrue, stmt.String(), stmt.GetLine(), []string{knownFact_paramProcessed.UniFact.String()})
			}
			return glob.NewEmptyVerRetTrue()
		}
	}
