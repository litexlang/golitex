	...
	for i := range stmt.Facts {
		...
		if verRet.IsTrue() {
			if state.WithMsg {
				...
				return glob.NewVerMsg2(glob.StmtRetTypeTrue, stmt.String(), glob.BuiltinLine0, append([]string{msg}, verRet.VerifyMsgs...))
			}
...
func (ver *Verifier) verFactAtIndex_WhenOthersAreFalse(facts []*ast.SpecFactStmt, i int, state *VerState) *glob.VerRet {
	...
	for j := range facts {
		...
		if ret.IsErr() {
			return glob.NewVerMsg2(glob.StmtRetTypeError, facts[j].String(), glob.BuiltinLine0, []string{ret.String()})
		}
	}
