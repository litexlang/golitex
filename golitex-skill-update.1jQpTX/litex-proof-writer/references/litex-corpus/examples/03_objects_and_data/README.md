...
Each Litex block below is checked independently by `cargo test --release run_examples -- --nocapture`.
...
# --- Explicit restriction value ---
abstract_prop p(f fn(a Q, b Q) R)
have f fn(x R, y Q) R
$p(fn(a Q, b Q) R {f(a, b)})
...
## 20. `explicit_function_domain`

- Category: `obj`
- System surface: `anonymous function`
- Purpose: Shows explicit function values on smaller domains.

```litex
scratch:
    have f fn(x R, y Q) R 

    abstract_prop takes_two_rationals(g fn(a Q, b Q) R)
    $takes_two_rationals(fn(a Q, b Q) R {f(a, b)})

    have g fn(x, y R) R

    abstract_prop takes_ordered_pair(h fn(x, y R: x < y, x < 0) R)
    $takes_ordered_pair(fn(x, y R: x < y, x < 0) R {g(x, y)})

scratch:
    abstract_prop p(f fn(x closed_range(1, 2)) R)
    have f fn(x R) R
    $p(fn(x closed_range(1, 2)) R {f(x)})
...
scratch:
    abstract_prop p(f fn(x closed_range(1, 2)) R)
    $p(fn(x closed_range(1, 2)) R {x})
    $p(fn(x closed_range(1, 2)) R {x + 1})
```
