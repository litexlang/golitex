...
Function types are exact. If an interface expects `f fn(x E) R` and `g` has a
larger domain, pass the explicit restricted value `fn(x E) R {g(x)}`. This is
the ordinary spelling of `g | E`; Litex does not silently retype `g`.
