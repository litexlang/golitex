...
func RunFileStmtInExecutor(curExec *exe.Executor, importFileStmt *ast.RunFileStmt) glob.GlobRet {
	...
	stmtSlice, err := ast.ParseSourceCode(code, curExec.Env.EnvPkgMgr.PkgMgr.CurPkgDefaultName, curExec.Env.EnvPkgMgr.PkgMgr, curExec.Env.EnvPkgMgr.PkgMgr.CurRepoAbsPath)
	if err != nil {
		return glob.NewGlobErr(err.Error())
	}
