...
当然如果一坨符号是在做整数或者实数的 操作，那我默认会帮用户验证一下，而不是用户手动需要说明一下怎么证明

之所以不引入id做“所有几何上的等同函数”，而是让每个集合都有 setName_id 这个函数，本质还是我不想让函数名重载；+-*/ 也只是在某几个集合上被定义了；你如果想让 id 在所有的集合上定义，难度比重载 +- * / 还要大；另外 != 和 = 是可以读入所有的类型的东西的, (!= 等价于 not =)
...
5.15
1. 如果 prop 不要求 用户输入 来自什么集合，其实也行。也不会让整个语言不严格。只要我的语言里出现的obj，fn是来自某些集合的，那fn能作用的对象，至多是来自这些集合的并的，所以也是可以的。
或者说 in 这个关键词的prop和其他类型的prop本质上是一样的。没必要让它这么突出。我可以让litex像py一样，相对来说更无类型一点（只要引入obj和fn的时候固定类型就行）

2. 无类型让整个系统非常非常灵活
比如用户和我都可以轻松引入函数 id 让 id 能读入任何类型的东西并返回自己： id(x) = x 。相当于这种函数是在对string做操作，而不是对有某种意义的东西做操作。

数学说是有类型的（或者说prop，fn都需要规定定义域），但世纪生活中，他们用起来都是很generic的。比如continuous这个prop既能作用在1元实数值函数上，又能作用在2元实数值函数上，甚至能作用在任何拓扑空间。与其从下到上地，在定义prop的时候就规范好类型（比如在定义连续的时候，先定义好拓扑空间），不如“遇到需求了，再规范”。因为数学的发展，也不是上来就有拓扑空间这个概念的，也是先是一元，再是二元，最后抽象在集合上去发展连续的定义。
...
REMARK:
两种处理recursive方式，自上而下不断询问，自下而上确定地推出(自下而上通常是行为相对固定，每层跳跃都能成立)
后面这种其实也很常见，而且实用，因为它不需要forall了，每一层的forall就是直接执行完毕了。
...
重载 +-*/ 这种符号：如果处理的对象是R,N,C 那就按正常的去处理；如果处理的对象是 返回值是R,N,C 的函数，那就按返回值是R,N,C 的函数去处理

REMARK
不可调和的矛盾
我可以渐渐的让litex能验证所有的事实（一阶逻辑，高阶逻辑，甚至能图灵机而非 只读图灵机地 验证）
但我不能让同一个符号重载多种意思，也不能让 不同的“默认”的事实 在不同的场景下用户啥都不说 就让这些默认的东西自动运行（我只能让它们用尽可能少的一句话，自动运行，但你想啥都不说那我也不知道你要做什么）
同一个符号，在不同的场景下，有不同的含义，这是我无论如何都不能解决的难题，也是自然语言得以比形式语言更自由的主要原因之一（另外一个原因是自然语言一直在跳步。虽说数学是严格的学科，但它写成自然语言其实是充满了跳步的，而且往往只有很少的人能看出来，所以很多错误会存在很多年才被发现，这也让数学变得很分裂，甚至有一言堂的趋势）。这种自由和严格性之间的平衡，是我要考虑的。

lean4 其实也很难解决x + x + x = 3 * x 这种问题，它需要用户写成
theorem add_three_times (x : Nat) : x + x + x = 3 * x := by
  rw [← Nat.two_mul]  -- 将前两个 x 相加转换为 2 * x
  rw [← Nat.add_mul]  -- 分配律：(a + b) * x = a * x + b * x
  simp                -- 简化：2 * x + 1 * x = 3 * x

lean4 如果要验证两个数相等，还要记住 norm_num 这个命令，这是很麻烦的。litex送给用户
example : 2500000 * 2500000 = 6250000000000 := by
  norm_num

REMARK
从递归的最底层出发，逐层到最外面的。这种处理方式，和自顶向下一层层地猜 每一层的是怎么验证是对的，分别是两种，也是唯二的两种处理 证明一个东西成立 的方式
例子：证明一个符号 f(g) 是可积的
自顶向下适用的场景，适合 符号的封装层很少的情况（比如群论里，通常不会函数套函数，都是一个atom在那里操作）；自下而上的验证，适用于 符号的嵌套很深，但是每一层向上emit事实又是很固定的套路，可以写一个go语言的loop+if-else来实现。这种场景也是很多的，比如分析里的符号都很深

从上往下地递归，说实话是很方便的，直接用原生的litex功能就行

自下而上地递归，需要用 prove_recursively 这个命令，甚至说接外界的go语言（或者python之类的，只要能自动地自下而上生成证明就行），来实现。

什么叫自动的自下而上的生成证明

比如 f(g(h(t))) 这是一个函数，要证明它是连续的。那我先说 t 连续，然后h(t) 连续，然后g(h(t)) 连续，然后f(g(h(t))) 连续。这里有很明显的规律：前一个符号，套上一层外层的函数名，就能得到下一个证明的语句。

REMARK
在用已知的等式命题做验证的时候，涉及到的，其实是无语义的符号的比较。只有在 得到这个等号 的过程中，才涉及到 符号的语义。一旦它作为一个事实被存下来了，它就是无意义的了，即没有语义了

+r, -r 可以用来表示在r上的+和-，这样就不需要重载+和-了

5.16
考虑把取负换成 ~ 而不是 - . 或者要么我可以允许重载函数名，但不能像C++那样重载，必须要是函数的参数的数量不同，才能重载

改动 in 的存储。让它存的像equal那样用map.这样也让运行效率高很多（当然，涉及到uni的事实，还是和其他东西一样存在list里)
...
5.25
用!做函数名的结尾，让它从集合S上的运作的东西，一下变成在返回值是S的函数上的运作的东西。
...
6.4
1. 1%的用户会涉及到底层数学，这是另外一大类的“不直观”来源，本质上这也不可避免，因为底层的东西我提供给用户很少，为了保持语言简单。但是用户如果非要对底层的东西做hack，那他要自己搞定。比如建立两个函数的等价，比如建立两个prop的等价
2. 需要时刻记住的是，verState的本质意义是，防止iff的循环验证。如果用户自己定义algo来做验证的时候，我允许他利用完全的图灵完备性，他可以自定义算法来验证的时候引入循环。
3. REMARK：验证数学不需要图灵完备，但生成证明的人或者机器，可以是图灵完备的。
4. 关于如何设计 algo
algo：能递归去验证，能给这个algo自己取名，能读string（可以处理多元一次方程组），相当于在litex里有个图灵完备的生成证明的机器。让一个algo在特定场景下，被触发工作。比如验证 integral里的东西是不是可积的时候，就自动运行，一步步迭代出来。这相当于 “现有目标，然后用指定的方式去验证，理论上可以recursively地验证无数层”。而不是像litex正常工作的那样”有了目标，遍历所有的fact，然后两层的验证“。

6.15
必须要实现 fn_set 了。即一类函数
类似template
fn_set f(params):
    dom_of_params
    iff:
...
2. 如果不想让in有这两种情况的判定，那就让 set 和独立的谓词 is 配合，即 能 写成 xxx in yyy 的 yyy 必须要是 yyy is set。然后 is 的要求是，只能 右侧字面量（或者说=意义下的字面量）形如 set，其他全不行
    1. 但这时候 in 和 set 的联合起来的用途 和 复用谓词in的情况一样了，没有变化
3. 整个需要区分对待的想法，来自C++的左值和右值性质不一样。虽然没什么类似性，但我确实是从这个地方想到 所谓 左参数和有参数可能的处理方式不同的。
