...
export namespace executor {
  ...
  export function nodeExec(env: L_Env, node: L_Node): RType {
    try {
      ...
      if (execFunc && isRTypeTrue(execFunc(env, node)))
        ...
        return shortOptExec(env, node as ShortCallOptNode);
      } else if (node instanceof IfThenNode) {
        return ifThenExec(env, node as IfThenNode);
      }
  ...
  function ifThenExec(env: L_Env, node: IfThenNode): RType {
    const res = checker.checkIfThen(env, node as IfThenNode);
    if (isRTypeTrue(res)) {
  ...
  function shortOptExec(env: L_Env, node: ShortCallOptNode): RType {
    const res = checker.checkShortOpt(env, node as ShortCallOptNode);
    if (isRTypeTrue(res)) {
      //! Here is where a new fact is generated by previous facts
      // equivalent to env.addShortOptFact(env, node, []);
      knowExec(env, new KnowNode([node]));
      return successMesIntoEnv(env, node);
    } else if (res === RType.False) {
      env.newMessage(`Unknown. ${node.toString()}`);
