...
AI 的到来正在迅速增加机器生成的数学证明。AI 可以很便宜地生成定理名、tactic 调用和例行边界条件代码；但生成成本降低，并不自动意味着人类理解、修改和审查最终产物的成本也降低。验证不再是唯一的瓶颈，数学理解本身仍然是瓶颈。

> AI 可以负责补充例行的机器细节，但长期保存下来的源码应当仍是人能辨认的数学，并且它的验证过程可以按需检查。

Litex 要检验的假设是：能否设计一门像日常数学一样以对象和事实为中心、但仍由机器严格检查的形式化语言，让写下和验证数学之间的距离足够短，让使用者真的能理解自己正在使用的形式化工具、它的可信边界，以及一条结论究竟凭什么通过。Litex 的语言设计和输出都应当服务于这个更严格的问题。只有当更少的 proof command 真正降低理解成本，而不是把证明藏进不透明的自动化时，这种简化才有意义。
