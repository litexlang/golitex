...
> **任何 Litex 中的工作都应能转化为 Lean 中的工作；Lean 和 Mathlib 的进步也应能被 Litex 接入。两者不必各自生长，而可以共同进步。**
...
这仍然是 Litex 正在实现和检验的目标，而不是当前 beta 版本已经全面达成的能力。现阶段的 Litex-to-Lean 编译器仅覆盖部分验证路径；它将继续扩大语义覆盖，使映射可审计、可重放，并在遇到尚不支持的路径时明确失败，而不是用隐式公理或未完成证明填补缺口。
