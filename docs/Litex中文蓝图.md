...
这个例子的解释会分布在下面五个 section 中，不再在这里重复一轮五点总结。这样，代码对照和设计论证只保留一套对应关系。
...
Lean 源码没有把中间 Goal 写成定理内的数学陈述；光标沿 tactic 依次后移时，Infoview 把这一部分补出来。下面省略每一步都不变的 local context：
...
另一大区别是，**Litex 用户声明 *what*：“什么应当成立”；Lean tactic 用户声明 *how*：“应当怎样证明 Goal”。** Litex checker 寻找能与结果匹配的证明依据，并解释找到的验证路径。Lean tactic elaboration 按照用户的证明指令构造对应的 proof term，server 显示变化后的 Goal，kernel 检查该 term。
