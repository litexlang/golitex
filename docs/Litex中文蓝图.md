...
1. **降低可检查形式化的成本。** 通过让作者记录数学证明的主干、由 checker 重建常规的局部联系，Litex 试图让更多数学在投入上值得被形式化。真正决定这一价值的指标，是同一真实任务上的端到端总投入，而不是源码行数本身。
2. **为人和 AI 的协作提供验证接口。** 人或 AI 可以提出下一条数学事实，获得局部且可机器检查的反馈，再修复确切的失败边界；已经通过的事实和修复轨迹可以成为可复用的形式化数据，而不只是看似合理的文本。这一价值取决于验证结论及其来源是否可信。
3. **让数学文本可执行。** 定义、引理和证明可以在保持数学叙述可读的同时，跨文件、跨章节接受检查和复用。这有机会让形式化验证走近教材、教学和科学写作，而不只存在于面向专家的证明助手工程中。
4. **把形式化失败转化为研究证据。** 一次边界清楚的失败，可以暴露语言能力、标准库事实、推理路径、内核能力、诊断或数学表述上的缺口。只有当失败与可信缺口被明确记录而不是被隐藏时，这类证据才真正有价值。

这些价值都有成立条件，不会仅仅因为语法可读就自动出现。[附录](#appendix-conditions)总结了 Litex 已经开始投入的具体工作，以及仍然开放的边界。
...
总结中的四层价值不会由 Litex 的接口自动产生。当前 beta 已经沿着下面六个方向投入工作，用真实实现和语料检验这些价值的成立条件；这些仍是进行中的努力，不是已经完成的可靠性、可扩展性、易用性或性能证明。

1. **让可信边界可见，并逐步接受独立复核。** `trust`、`trust have` 和 `axiom` 都是显式源码形式；普通文件运行会报告尚未验证的导入，而 `-strict` 会验证配置依赖并拒绝这些可信形式。常规输出和详细输出会区分一条事实为何通过、什么进入了上下文、随后推断出了什么，以及出现了哪些直接可信语句。验证路径正在携带越来越结构化的证据，使部分路径能够被 Litex-to-Lean 编译器交给 Lean kernel 重放；不支持的路径会明确失败，而不会生成 `sorry` 或新公理。当前边界仍然重要：Litex 会记录直接可信语句，但还没有为每条下游事实传播完整的传递性 trust 标签；Lean 重放也只覆盖已有文档说明的部分语言与 builtin rules。参见[系统地图](Litex_System_Map.md#reading-results-and-trust)、[CLI 可信控制](cli.md#global-options)和[编译器当前边界](../lean/README.md)。

2. **用长篇数学持续检验组合能力。** Litex 的模块系统为 import 和 export 规定确定的数学顺序，同时支持聚焦的单文件检查和完整模块运行。已登记的多文件教材工作区会持续检验定义、定理、命名空间和依赖能否跨章节组合，而不是只在孤立小例子中成立。这些项目是持续压力测试，不是“任意长篇工程已经能顺畅扩展”的证明；参见[模块约定](cli.md#project-modules)和[教材工作区登记表](../scripts/.textbooks)。

3. **不让自然的源码变成不可解释的魔法。** 一条简洁事实可以触发局部依据重建，但阅读视图与审计视图会展示直接的 `why_verified` 原因、良定义性和验证阶段、被引用事实、builtin-rule 前提与失败边界；关系图和事实图还会从依赖角度展示这些路径。Builtin rules 以明确的数学模式和具名证据实现，工作目标还包括聚焦的正向回归与邻近负向边界，而不是提供一个无法区分内部路线的自动化按钮。这些工具让被源码隐去的工作可以被检查，但它们本身不能证明每条规则正确，也不能保证当前每条解释都已经足够易懂；参见 [CLI 输出约定](cli.md)和[英文版附录 A](Litex_Blueprint.md#appendix-builtins)。

4. **降低并实际测量形式化的总成本。** 以事实为中心的语言表层、有界局部证明搜索、紧凑反馈、登记文件运行和持久化的 `-session -before` 工作流，都在尝试减少证明书写与修复的投入和等待时间。Litex 目前还没有通过受控的同题端到端实验，证明自己相对成熟方案具有成本优势；这项主张需要在代表性数学上测量作者时间、修复轮次、证明债务、运行时间和维护成本，而不能只依赖几个较短的代码对比。

5. **接入 Lean，而不是形成新的知识孤岛。** 仓库已经包含部分 Litex-to-Lean 编译器、Lean 语义包装层、成对的生成示例和真实 Lean kernel 检查。对已经支持的路径，编译器会保留 verifier 所有的事实身份、作用域、证明证据和良定义性义务；遇到尚不支持的已验证语句时，它会拒绝编译，而不是静默发明证明。覆盖范围仍然明显不完整，因此互操作目前是一条不断扩展的审计与迁移路径，不是一般性的导出保证；参见[通向 Lean 可信生态的简洁数学前端语言](#compatibility)和[编译器当前支持边界](../lean/README.md)。

6. **使用真实语料，并保存失败证据。** Litex 正在接受普通示例、多文件教材和数据集工作区的检验，其中包括 MATH500、miniF2F、中学数学以及更长的本科数学内容。数据集 runner、源工作区中的 todo、证明日志、trust 审计和最小回归用例，用来区分已经检查的结果与只完成翻译但仍有缺口的工作，并把失败转化为语言、标准库、推理、诊断或内核任务。语料规模本身不是能力证据：只有通过对应登记 gate 的条目才能作为已检查证据，混合状态或尚未完成的语料必须继续明确标注；参见[英文版附录 B](Litex_Blueprint.md#appendix-examples)和[数据集 runner 源码](../src/main_test/lit_file_runner_tests/dataset_runners.rs)。

这些努力共同构成一项工程与研究计划，而不是宣称六个困难条件已经全部满足。只有当可信边界持续可审计、长篇数学继续能够组合、解释始终可检查、真实任务的端到端成本下降、Lean 重放在不引入证明缺口的前提下扩大，并且真实失败保持可见时，Litex 才真正获得前述四层价值。
