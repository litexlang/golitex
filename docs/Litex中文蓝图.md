...
沿着上面两个工作流差异来看，这个例子引出的是五个相互连接的设计目标，而不是一组彼此独立的语法特性。

1. 用户直接声明结果，系统寻找证明依据

这是 Litex 与典型 Lean tactic 交互最根本的分工差异。Litex 用户主要声明“结果应当是什么”，checker 再根据事实的形状，从 builtin 规则、当前上下文和已知全称事实中寻找证明依据。Lean 用户在 tactic 模式中则主要声明“这个 Goal（结果）应当怎样被改写或分解”，系统按这些指令构造并检查最终的 proof term。
...
`Group` 的运算规律直接写在结构定义的 `<=>:` 部分，不必先给它们分别取名。单位元唯一性的证明也按数学叙述展开：用户直接声明候选元素 `identity` 满足的左右单位律，再声明结果链 `identity = G.mul(G.one, identity) = G.one`。用户提交的是应当成立的数学结果，不是一串改变 proof state 的操作命令。
...
一个完全不涉及计算的例子是让成员关系沿集合包含关系传递。Lean 从目标 `x ∈ c` 出发，通过 `apply` 逐步声明应当怎样证明它：
...
输出则把两种源码各自没有写出的部分显示出来。在 Lean 中，光标依次位于这些 tactic 附近时，Infoview 会展示 `x ∈ c`、`x ∈ B`、`x ∈ A` 和最后的 `no goals`。在 Litex 中，`x $in B` 和 `x $in c` 已经是源码的一部分；runner 输出因而报告它们的 `why_verified`，这里两步都由“membership through a known direct set inclusion”这条直接包含规则验证。
...
> 一份完整的乐高说明书同时包含两类信息：第一，这
> 一步应当怎样拼；第二，这一步完成后，整个半成品应当是什么状态。Lean
> tactic 源码主要记录第一类信息——下一步怎样操作 proof state；Litex
> 源码主要记录第二类信息——这一步推理后得到了什么数学事实。
