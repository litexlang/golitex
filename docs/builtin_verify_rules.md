...
**Finite sum — structural rules (6–10):** peel / additivity / single index / split / factor-scalar use **`objs_equal_by_display_string`** where noted (same object **`Display`** text), not **`verify_objs_are_equal`**. Rule **5** (two sums) uses **`verify_objs_are_equal`** on bounds and on summands inside a local scope. There are **no** dedicated finite **`product(...)`** equality builtins in this pipeline (use generic equality / calculation / rational simplification, or prove otherwise).
...
7. **Finite sum (additivity, same bounds)** — **`sum(i, a, b, F+G) = sum(i, a, b, F) + sum(i, a, b, G)`** (one **`+`** on the other side; the two inner sums may be swapped). Reason: `equality: sum(summand + summand) = sum + sum same bounds`.
8. **Finite sum (single index)** — If **`start`** and **`end`** match by **`objs_equal_by_display_string`**, then **`sum(i, a, b, F) = inst_obj(F, { i ↦ a }, Sum)`** (RHS must match the instantiated body the same way). Reason: `equality: sum with start = end is single instantiated summand`.
9. **Finite sum (split into two adjacent segments)** — **`sum(i, a, b, F) = sum(i, a, k, F) + sum(i, k+1, b, F)`** (order of the two sums around **`+`** either way). Same **`i`**, **`a`**, **`b`**, **`F`**; **`first_end + 1`** vs **`second_start`** by **`objs_equal_by_display_string`**. Reason: `equality: sum splits into adjacent segments (end+1 = next start)`.
10. **Finite sum (factor scalar)** — **`sum(i, a, b, k*F) = k * sum(i, a, b, F)`** (either **`*`** order in the sum body and on the other side). Same **`i`**, **`a`**, **`b`**, **`F`**, **`k`** by **`objs_equal_by_display_string`**; **`k`** must be well-defined and not use the outer sum index (lexical binding check). Reason: `equality: sum(k * summand) = k * sum(summand) with k well-defined and independent of sum index`.
11. **Same + calculation** — `verify_equality_by_they_are_the_same_and_calculation` (identity and partial evaluation).
12. **Rational simplification** — If still plausible, **`objs_equal_by_rational_expression_evaluation`** on evaluated pair; reason `calculation and rational expression simplification`.
13. **Two `fn` set values** — `verify_fn_set_with_params_equality_by_builtin_rules` (structural compare).
