...
**`sum` / `product` (closed integer range):** additivity on one range (pointwise `+` on the summand body); merge adjacent index intervals into one range; **split off the last index** — **`sum(s,e,f) = sum(s,e-1,f) + f(e)`** and **`product(s,e,f) = product(s,e-1,f) * f(e)`** when both sides use the same unary anonymous `f`, the same start `s`, and the longer end satisfies **`e = (e-1) + 1`** relative to the shorter range (full `verify_objs_are_equal` checks the last term vs `f(e)`); **partition** — **`sum(s,e,f) = sum(s1,e1,f) + … + sum(sk,ek,f)`** (and the `product` analogue with `*`) when the sub-ranges are **edge-to-edge** (`end_i + 1 = start_{i+1}`), tile **`[s,e]`** (`s1 = s`, `ek = e`), every piece is a `sum` / `product` with the **same** `f`, and there are **at least two** pieces; reindexing by a fixed shift; constant summand over a range. Builtin reasons include **`equality: sum through e equals sum through e-1 plus last summand f(e)`**, **`equality: product through e equals product through e-1 times last factor f(e)`**, **`equality: sum partitions closed range into adjacent sub-sums with the same summand`**, **`equality: product partitions closed range into adjacent sub-products with the same factor`**.
...
5. **Integer mod (`%`)** — After sum-range builtins, before same/calculation:
   - **Nested mod:** **`((x % m) % m) = (x % m)`** when the inner and outer moduli are provably the same as on the other side (absorption of an inner **`% m`**). Reason: **`equality: nested mod with same modulus absorbs inner mod`**.
   - **Peel nested `% m`:** **`a % m = ((b % m) % m)`** (or symmetric) reduces to proving **`a % m = b % m`** when the inner modulus matches **`m`**; recursion uses the full equality pipeline so **`+`** / **`*`** / **`%`** congruence can apply again. Reason: **`equality: mod — peel outer nested % m to reuse residue equality (full verify_objs_are_equal)`**. This covers left-associative chains such as **`b % m * c % m % m`** versus **`(b * c) % m`** inside a larger goal.
   - **Congruence:** **`(a op b) % m = (a' op b') % m`** for **`op`** in **`+`**, **`-`**, **`*`**, when **`m`** matches on both sides and **`(a % m) = (a' % m)`** and **`(b % m) = (b' % m)`** are each provable (full equality pipeline on those two goals). Reason: **`equality: integer congruence — same modulus, residues for matching + / - / *`**. Together with nested mod, this supports identities like **`(x+y)%m = ((x%m)+(y%m))%m`**.
