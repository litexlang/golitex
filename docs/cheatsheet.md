...
| `setting Name: ...` | Binder lines must come first, followed by optional shared-condition facts; the name must be unused. | None. Each `forall [Name]` use elaborates to an ordinary universal fact, where normal well-definedness and proof checks apply. | Stores an elaboration-only parameter/domain bundle; every use receives fresh binder identities. |
