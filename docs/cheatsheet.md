...
| `have fn by induc` | Function and induction shapes are checked. | Proves the measure and lower bound are integer-valued, then verifies the lower bound, case partition, return values, and strict decrease of recursive calls. A later provable case is instantiated with nested arithmetic normalization. | Stores the function definition facts; unknown case conditions do not unfold. |
...
| Statement | Well-Definedness / Structural Checks | Truth Verification | Environment Effects |
|---|---|---|---|
| `prop` | Parameters and `iff` facts must be well-defined; prop and abstract-prop names must not conflict. A header may mix ordinary typed parameters with `[Setting]` or `[Setting(fresh_names)]`; every listed name is a fresh declaration, exact arity is required, and setting conditions are prepended in header order. | Does not prove the `iff` facts. | Stores the concrete prop definition after elaborating setting bundles to its flat parameters and facts. |
| `abstract_prop` | Abstract-prop and concrete-prop names must not conflict. | None. | Stores the abstract prop definition. |
| `struct` | Parameter domains and field types must be well-defined; a header may mix typed parameters with setting bundles; bundle conditions are prepended to equivalent facts, which are checked left to right in a temporary field scope; struct name must be unused. | Does not prove equivalent facts. | Stores the flat header parameters, fields, and membership conditions only after the complete check succeeds; bundle parameters do not become fields. |
...
| `setting Name(params): ...` | The header may mix typed parameters with previously declared setting bundles; the body contains only shared-condition facts. Omit the colon and body when there are no shared conditions. The name must be unused. In `[Name(fresh_names)]`, arguments are bare fresh binder declarations, not outer references or expressions, and their arity must match exactly. | None. Each block/inline `forall`, concrete `prop`, nested `setting`, or parameterized `struct` use elaborates to ordinary parameters and facts, where normal well-definedness checks apply. | Stores an elaboration-only flat parameter/domain bundle; every use receives fresh binder identities. |
