...
| `obj &StructName` in a binding, then `obj.field` (preview) | Select a default struct view with an explicit binding type, then use parse-time field shorthand. | The parser lowers `obj.field` to `&StructName{obj}.field`. A later fact `obj $in &StructName` does not select a default view. |
...
binding selects that view; a membership fact does not. Use the fully explicit
`&StructName{obj}.field` form to select another view at one access.
