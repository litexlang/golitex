...
| Reduction bridges | Reuse existing aggregate, function, and set interfaces. | Additive/zero and multiplicative/one reductions equal the corresponding sum/product. `$fn_eq_in` gives congruence, `$bijective` gives finite-set reindexing, adjacent ranges compose in order, and disjoint unions compose by nesting one reduction as the other's seed. |
