...
`verification`. A successful `forall` fact has a more structured shape:
`parameters`, `assumptions`, and `conclusions_with_verification`, where each
conclusion carries its own `verification` object.
