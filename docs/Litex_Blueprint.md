...
2. **Test composition across long mathematical developments.** A clear trust boundary is useful only if definitions and proofs continue to compose beyond small examples. Litex's module system gives imports and exports a deterministic mathematical order and supports both focused file checks and complete module runs. Registered multi-file textbook workspaces exercise definitions, theorems, namespaces, and dependencies across chapters. They provide practical evidence about current composition behavior while leaving open how smoothly arbitrary large developments will scale; see the [module contract](cli.md#project-modules) and [registered textbook workspaces](../scripts/.textbooks).

3. **Keep convenience inspectable.** As local reconstruction becomes more capable, concise source must not turn into unexplained automation. The reading and audit views expose immediate `why_verified` reasons, well-definedness and verification phases, cited facts, builtin-rule premises, and failure boundaries; relation and fact graphs provide another view of those dependencies. Builtin rules are implemented as explicit mathematical patterns with named evidence, with focused positive regressions and nearby negative boundaries as the audit target. These tools make more of the checker's work inspectable, even though rule review and the clarity of every explanation remain works in progress; see the [CLI output contract](cli.md) and [Appendix A](#appendix-builtins).

4. **Reduce and measure the total cost of writing and review.** Trust and inspectability matter in practice only if real proofs also become economical to produce and maintain. The fact-oriented surface, bounded local proof search, compact feedback, registered-file execution, and persistent `-session -before` workflow are attempts to reduce the effort and latency of writing, repairing, and reviewing proofs. A convincing same-task advantage over mature alternatives still requires measurements of author time, repair iterations, the effort needed to audit whether AI-generated statements express the intended mathematics, proof debt, runtime, and maintenance on representative tasks.

5. **Let Litex results continue into the Lean ecosystem.** Lowering the entry cost should not isolate the resulting mathematics from more mature systems. The repository contains a partial Litex-to-Lean compiler, a Lean semantic wrapper, paired generated examples, and real Lean kernel checks. For supported routes, the compiler retains verifier-owned fact identities, scopes, proof evidence, and well-definedness obligations; unsupported verified statements are rejected rather than being completed with invented proof. Coverage remains deliberately partial, so interoperability is currently a growing audit and migration path rather than a general export guarantee; see [Litex: A Concise Mathematical Front-End Language for the Trusted Lean Ecosystem](#compatibility) and the [compiler's current support boundary](../lean/README.md).
