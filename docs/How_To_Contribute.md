...
Use this structured record for every new or modified translation item:
...
- `natural_language_idea`: the mathematical proof idea before Litex code.
- `litex_code`: the current runnable or intended Litex code.
- `proof_attempt`: what was tried, especially if the proof is partial.
- `status`: `translated`, `checkable`, or `blocked`.
- `blocker`: one primary blocker label if status is `blocked`; otherwise empty.
- `notes`: source caveats, license notes, verifier command, or follow-up work.

Do not submit only raw Litex code. If an item is blocked, record the exact
reason and the smallest useful reproduction. If the source text cannot be
redistributed, write a reusable mathematical reformulation and record the
license concern in `notes`.
...
- `id`：稳定编号，在当前 source 内唯一。
- `source`：数据集、书、考试、竞赛或来源名称。
- `topic`：代数、不等式、数列、集合、几何、数论等。
- `difficulty`：easy/medium/hard 或 1-5，遵循当前 source 的本地约定。
- `natural_language_idea`：写 Litex 之前的数学证明思路。
- `litex_code`：当前可运行或计划中的 Litex 代码。
- `proof_attempt`：尝试过的证明路线，尤其是 partial proof 或 blocked proof。
- `status`：`translated`、`checkable` 或 `blocked`。
- `blocker`：如果 status 是 `blocked`，这里写一个主要 blocker label；否则留空。
- `notes`：来源注意事项、版权/许可备注、验证命令或后续工作。

不要只提交裸 Litex 代码。如果条目 blocked，要记录精确原因和最小有用复现。
如果原始题面不能重新分发，写一个可复用的数学改写版本，并在 `notes` 里记录
license concern。
