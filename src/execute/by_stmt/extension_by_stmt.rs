...
impl Runtime {
    pub fn exec_by_extension_stmt(
        ...
        let local_proof_result =
            self.run_in_local_env(|rt| rt.exec_by_extension_stmt_in_local_env_body(stmt))?;
    ...
    /// 在局部环境中：执行 proof、验证左右互包含对应的 `forall` 事实，返回 proof 结果与两条 forall（调用方只需子集关系，可丢弃 fact）。
    fn exec_by_extension_stmt_in_local_env_body(
        ...
        let unused_name = self.generate_random_unused_name();
