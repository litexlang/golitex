    ...
    fn build_fn_characterization_facts(
        ...
        for param_def_with_set in fn_set.params_def_with_set.iter() {
            ...
            let instantiated_set = self
                .inst_obj(&param_def_with_set.set, &original_param_to_forall_obj, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
                .map_err(|inst_error| {
                    short_exec_error(
                        stmt_exec.clone(),
        ...
        let forall_ret_set = self
            .inst_obj(fn_set.ret_set.as_ref(), &original_param_to_forall_obj, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
            .map_err(|inst_error| {
                short_exec_error(
                    stmt_exec.clone(),
        ...
        for param_def_with_set in fn_set.params_def_with_set.iter() {
            ...
            let instantiated_set = self
                .inst_obj(&param_def_with_set.set, &original_param_to_exist_obj, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
                .map_err(|inst_error| {
                    short_exec_error(
                        stmt_exec.clone(),
        ...
        let exist_ret_set = self
            .inst_obj(fn_set.ret_set.as_ref(), &original_param_to_exist_obj, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
            .map_err(|inst_error| {
                short_exec_error(
                    stmt_exec.clone(),
        ...
        let forall_exist = ForallFact::new(
            ParamDefWithType::new(exist_param_defs_with_type),
            {
                ...
                for dom_fact in fn_set.dom_facts.iter() {
                    dom_facts.push(
                        self.inst_or_and_chain_atomic_fact(dom_fact, &original_param_to_exist_obj, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
                            .map_err(|inst_error| {
                                short_exec_error(
                                    stmt_exec.clone(),
                                    format!(
