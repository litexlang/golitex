...
impl Runtime {
    pub fn exec_claim_stmt(&mut self, stmt: &ClaimStmt) -> Result<StmtResult, RuntimeError> {
        let prechecked_well_definedness = self.exec_claim_stmt_verify_well_definedness(stmt)?;
        let result = self.exec_claim_stmt_verify_process(stmt, &prechecked_well_definedness)?;
    ...
    fn exec_claim_stmt_verify_well_definedness(
        ...
        if let Fact::ForallFactWithIff(_) = &stmt.fact {
            unreachable!("claim forall with iff is not supported");
        }

        let verify_result = match &stmt.fact {
            ...
            _ => self
                .verify_fact_well_defined(&stmt.fact, &UseContextVerifyState::new(0, false))
    ...
    fn exec_claim_stmt_verify_process(
        &mut self,
        stmt: &ClaimStmt,
            ...
            Fact::ForallFact(forall_fact) => {
                let body_exec_result = self.run_in_local_env(|rt| {
                    ...
                    for (then_index, then_fact) in forall_fact.then_facts.iter().enumerate() {
                        let mut result = rt
                            .verify_exist_or_and_chain_atomic_fact(then_fact, &then_verify_state)?;
                        if result.is_unknown() {
