...
const CATALOG_ACCEPTANCE_SOURCE: &str =
    include_str!("../../examples/05_compiler_interop/to_lean_local_builtin_catalog.lit");
...
fn on_large_stack(test_name: &str, test: impl FnOnce() + Send + 'static) {
    std::thread::Builder::new()
        ...
        .spawn(test)
        .expect("spawn To-Lean catalog test")
        .join()
        .expect("To-Lean catalog test panicked");
}

#[test]
fn catalog_acceptance_source_emits_all_registered_wrappers() {
    on_large_stack("to-lean-local-builtin-catalog", || {
        let lean = to_lean_from_source(
            CATALOG_ACCEPTANCE_SOURCE,
            "to_lean_local_builtin_catalog.lit",
        )
...
fn zero_one_and_two_premise_local_builtins_emit_registered_wrappers() {
    on_large_stack("to-lean-local-builtins", || {
        let lean = to_lean_from_source(ZERO_ONE_TWO_PREMISE_SOURCE, "local_builtin_catalog.lit")
...
fn registered_local_builtin_output_compiles_with_real_mathlib() {
    on_large_stack("to-lean-local-builtins-mathlib", || {
