...
impl Runtime {
    ...
    fn verify_fn_set_with_params_directionally_in_local_env_body(
        ...
        let source_ret_set = self
            .inst_obj(&source.ret_set, &source_param_to_generated_arg_map, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
            .map_err(|e| {
        ...
        let target_ret_set = self
            .inst_obj(&target.ret_set, &target_param_to_generated_arg_map, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
            .map_err(|e| {
                fn_set_equality_verify_error(
                    source,
                    target,
    ...
    fn define_directional_source_fn_set_params_in_local_env(
        ...
        for param_def_with_set in source.params_def_with_set.iter() {
            ...
            let instantiated_param_set = self
                .inst_obj(&param_def_with_set.set, &source_param_to_generated_arg_map, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
                .map_err(|e| {
                    fn_set_equality_verify_error(
                        source,
                        target,
    ...
    fn assume_directional_source_fn_set_dom_facts_in_local_env(
        ...
        for dom_fact in source.dom_facts.iter() {
            let instantiated_dom_fact = self
                .inst_or_and_chain_atomic_fact(dom_fact, source_param_to_generated_arg_map, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
                .map_err(|e| {
                    fn_set_equality_verify_error(
                        source,
                        target,
    ...
    fn verify_directional_target_fn_set_param_membership_facts(
        ...
        for param_def_with_set in target.params_def_with_set.iter() {
            let instantiated_param_set = self
                .inst_obj(&param_def_with_set.set, target_param_to_generated_arg_map, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
                .map_err(|e| {
                    fn_set_equality_verify_error(
                        source,
                        target,
    ...
    fn verify_directional_target_fn_set_dom_facts(
        ...
        for dom_fact in target.dom_facts.iter() {
            let instantiated_dom_fact = self
                .inst_or_and_chain_atomic_fact(dom_fact, target_param_to_generated_arg_map, (FreeParamObjType::FnSet, FreeParamObjType::FnSet))
                .map_err(|e| {
                    fn_set_equality_verify_error(
                        source,
                        target,
