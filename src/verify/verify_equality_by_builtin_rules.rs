...
impl Runtime {
    ...
    // Aligned parts use objs_equal_by_display_string (same as sum peel).
    ...
    fn try_match_product_multiplicativity_one_direction(
        ...
        for (x_side, y_side) in [
            ...
            if let (Obj::Product(px), Obj::Product(py)) = (x_side, y_side) {
                ...
                if !objs_equal_by_display_string(outer.start.as_ref(), px.start.as_ref()) {
                    continue;
                }
                if !objs_equal_by_display_string(outer.start.as_ref(), py.start.as_ref()) {
                    continue;
                }
                if !objs_equal_by_display_string(outer.end.as_ref(), px.end.as_ref()) {
                    continue;
                }
                if !objs_equal_by_display_string(outer.end.as_ref(), py.end.as_ref()) {
                    continue;
                }
    ...
    // product(i,a,a,F) = inst(F, { i ↦ a }) when start/end and RHS vs inst body match by objs_equal_by_display_string.
    ...
    // Matching uses objs_equal_by_display_string (same as sum split).
            ...
            if let Obj::Mul(mul) = right {
                for (p1_side, p2_side) in [
                    ...
                    if let (Obj::Product(p1), Obj::Product(p2)) = (p1_side, p2_side) {
                        ...
                        if !objs_equal_by_display_string(lprod.start.as_ref(), p1.start.as_ref())
                        {
                            continue;
                        }
                        if !objs_equal_by_display_string(lprod.end.as_ref(), p2.end.as_ref()) {
                            continue;
                        }
                        if !objs_equal_by_display_string(lprod.body.as_ref(), p1.body.as_ref())
                        {
                            continue;
                        }
                        if !objs_equal_by_display_string(lprod.body.as_ref(), p2.body.as_ref())
                        {
                            continue;
                        }
                        let first_end_plus_one: Obj =
            ...
            if let Obj::Mul(mul) = left {
                for (p1_side, p2_side) in [
                    ...
                    if let (Obj::Product(p1), Obj::Product(p2)) = (p1_side, p2_side) {
                        ...
                        if !objs_equal_by_display_string(rprod.start.as_ref(), p1.start.as_ref())
                        {
                            continue;
                        }
                        if !objs_equal_by_display_string(rprod.end.as_ref(), p2.end.as_ref()) {
                            continue;
                        }
                        if !objs_equal_by_display_string(rprod.body.as_ref(), p1.body.as_ref())
                        {
                            continue;
                        }
                        if !objs_equal_by_display_string(rprod.body.as_ref(), p2.body.as_ref())
                        {
                            continue;
                        }
                        let first_end_plus_one: Obj =
