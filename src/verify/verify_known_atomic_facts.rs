...
impl Runtime {
    ...
    pub(crate) fn atomic_fact_with_explicit_definitions_resolved(
        &self,
        atomic_fact: &AtomicFact,
    ) -> Result<Option<(AtomicFact, EqualityTransportEvidence)>, RuntimeError> {
        let original_args = atomic_fact.args_ref();
        let definitions = self.known_object_definitions_for_objects(&original_args);
        if definitions.is_empty() {
            return Ok(None);
        }

        let mut resolved_args = original_args
            .iter()
            .map(|argument| (*argument).clone())
            .collect::<Vec<_>>();
        let mut used_definition_keys = HashSet::new();
        let mut used_definitions = Vec::new();

        loop {
            let mut made_progress = false;
            for definition in definitions.iter() {
                let Obj::Atom(atom) = &definition.defined else {
                    continue;
                };
                let Some(symbol) = atom.symbol_ref() else {
                    continue;
                };
                let substitution_key = symbol.substitution_key();
                if used_definition_keys.contains(&substitution_key) {
                    continue;
                }
                let substitution =
                    HashMap::from([(substitution_key.clone(), definition.value.clone())]);
                let replaced_args = resolved_args
                    .iter()
                    .map(|argument| self.inst_obj(argument, &substitution, ParamObjType::Forall))
                    .collect::<Result<Vec<_>, _>>()?;
                if resolved_args
                    .iter()
                    .zip(replaced_args.iter())
                    .all(|(before, after)| obj_equality_key(before) == obj_equality_key(after))
                {
                    continue;
                }
                resolved_args = replaced_args;
                used_definition_keys.insert(substitution_key);
        ...
        if used_definitions.is_empty() {
            return Ok(None);
        }
        let Some(resolved_fact) = Self::atomic_fact_with_replaced_args(atomic_fact, resolved_args)
        else {
            return Ok(None);
        };
        if nested_obj_binder_normalized_fact_key(&Fact::from(resolved_fact.clone()))
            == nested_obj_binder_normalized_fact_key(&Fact::from(atomic_fact.clone()))
        {
            return Ok(None);
        }

        let module_names = self.atomic_fact_referenced_module_names(atomic_fact);
        let mut source_to_goal_steps = Vec::with_capacity(used_definitions.len());
        for definition in used_definitions.into_iter().rev() {
            let equality_fact: Fact = AtomicFact::EqualFact(definition.equality.clone()).into();
            source_to_goal_steps.push(EqualityTransportStep::new(
                definition.value,
                definition.defined,
                definition.equality,
                self.fact_id_for_transport_fact(&equality_fact, &module_names),
        ...
        Ok(Some((
            resolved_fact,
            EqualityTransportEvidence::new(source_to_goal_steps),
        )))
    }
