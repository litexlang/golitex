...
pub(crate) fn compile_markdown_ledger_file_to_lean(
    ledger_path: &Path,
    output_path: &Path,
) -> Result<usize, String> {
    ...
    for example in examples {
        ...
        let generated =
            compile_to_lean_from_source(&example.source, &source_label).map_err(|error| {
                format!(
...
#[cfg(test)]
mod tests {
    use super::compile_markdown_ledger_file_to_lean;
    use std::path::Path;
