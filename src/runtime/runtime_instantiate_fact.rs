...
impl Runtime {
    pub fn inst_fact(
        ...
        Ok(match fact {
            ...
            Fact::OrFact(or_fact) => {
                Fact::OrFact(self.inst_or_fact(or_fact, param_to_arg_map, to_inst_param_type)?)
            }
            Fact::AndFact(and_fact) => {
                Fact::AndFact(self.inst_and_fact(and_fact, param_to_arg_map, to_inst_param_type)?)
            }
    ...
    pub fn inst_exist_or_and_chain_atomic_fact(
        ...
        Ok(match fact {
            ...
            ExistOrAndChainAtomicFact::AndFact(and_fact) => ExistOrAndChainAtomicFact::AndFact(
                self.inst_and_fact(and_fact, param_to_arg_map, to_inst_param_type)?,
            ),
            ...
            ExistOrAndChainAtomicFact::OrFact(or_fact) => ExistOrAndChainAtomicFact::OrFact(
                self.inst_or_fact(or_fact, param_to_arg_map, to_inst_param_type)?,
            ),
    ...
    pub fn inst_or_and_chain_atomic_fact(
        ...
        Ok(match fact {
            OrAndChainAtomicFact::AtomicFact(atomic_fact) => OrAndChainAtomicFact::AtomicFact(
                self.inst_atomic_fact(atomic_fact, param_to_arg_map, to_inst_param_type)?,
            ),
            ...
            OrAndChainAtomicFact::ChainFact(chain_fact) => OrAndChainAtomicFact::ChainFact(
                self.inst_chain_fact(chain_fact, param_to_arg_map, to_inst_param_type)?,
            ),
    ...
    pub fn inst_atomic_fact(
        ...
        Ok(match atomic_fact {
            AtomicFact::NormalAtomicFact(fact) => AtomicFact::NormalAtomicFact(
                self.inst_normal_atomic_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            ...
            AtomicFact::LessEqualFact(fact) => AtomicFact::LessEqualFact(
                self.inst_less_equal_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::GreaterEqualFact(fact) => AtomicFact::GreaterEqualFact(
                self.inst_greater_equal_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            ...
            AtomicFact::IsNonemptySetFact(fact) => AtomicFact::IsNonemptySetFact(
                self.inst_is_nonempty_set_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::IsFiniteSetFact(fact) => AtomicFact::IsFiniteSetFact(
                self.inst_is_finite_set_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::InFact(fact) => {
                AtomicFact::InFact(self.inst_in_fact(fact, param_to_arg_map, to_inst_param_type)?)
            }
            ...
            AtomicFact::NotNormalAtomicFact(fact) => AtomicFact::NotNormalAtomicFact(
                self.inst_not_normal_atomic_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            ...
            AtomicFact::NotGreaterFact(fact) => AtomicFact::NotGreaterFact(
                self.inst_not_greater_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::NotLessEqualFact(fact) => AtomicFact::NotLessEqualFact(
                self.inst_not_less_equal_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::NotGreaterEqualFact(fact) => AtomicFact::NotGreaterEqualFact(
                self.inst_not_greater_equal_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            ...
            AtomicFact::NotIsNonemptySetFact(fact) => AtomicFact::NotIsNonemptySetFact(
                self.inst_not_is_nonempty_set_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::NotIsFiniteSetFact(fact) => AtomicFact::NotIsFiniteSetFact(
                self.inst_not_is_finite_set_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            ...
            AtomicFact::NotIsCartFact(fact) => AtomicFact::NotIsCartFact(
                self.inst_not_is_cart_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::NotIsTupleFact(fact) => AtomicFact::NotIsTupleFact(
                self.inst_not_is_tuple_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::NotSubsetFact(fact) => AtomicFact::NotSubsetFact(
                self.inst_not_subset_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            AtomicFact::NotSupersetFact(fact) => AtomicFact::NotSupersetFact(
                self.inst_not_superset_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
            ...
            AtomicFact::NotRestrictFact(fact) => AtomicFact::NotRestrictFact(
                self.inst_not_restrict_fact(fact, param_to_arg_map, to_inst_param_type)?,
            ),
        })
    }
    ...
    pub fn inst_and_fact(
        ...
        for fact in and_fact.facts.iter() {
            facts.push(self.inst_atomic_fact(fact, param_to_arg_map, to_inst_param_type)?);
        }
    ...
    pub fn inst_forall_fact(
        ...
        for dom_fact in forall_fact.dom_facts.iter() {
            dom_facts.push(self.inst_fact(dom_fact, param_to_arg_map, to_inst_param_type)?);
        }
