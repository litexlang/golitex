...
## Current native-interface boundary

The 2026-08-05 audit distinguishes construction predicates from consumer
carriers and source-facing names from duplicate low-level encodings. Current
chapters use builtin binary set operations, `fn_range` for restricted images,
native function-property predicates, refined `MeasurableSpace` consumers, and
checked decreasing recursion for natural-number folds and the tail-Fibonacci
state. Chapter-specific names such as `set_image` remain when they are useful
mathematical interfaces, but their implementations use the native object.
...
```text
candidate data -> *_laws -> structure object -> residual property/theorem
```
...
`AdditiveCommutativeGroupSetting`, `GroupSetting`, `RingSetting`,
`LatticeSetting`, and `MetricSpaceSetting`. Inside their theorem bodies the
operations are the direct subjects: `add(a, b)`, `mul(a, b)`, `meet(a, b)`,
and `dist(x, y)`.
...
This spreads only `add`, `zero`, and `neg`, in declaration order. The carrier
parameter and the struct law clauses are not positional arguments.

## Algebra objects and construction laws

The retained Chapter 2 module packages a generic partial order as a one-field
object. Its associated strict comparison is a residual relation on that object:
...
to non-strict comparison together with inequality. Strict comparison remains
a property because it introduces no selected value; the reusable order itself
is the object.

The retained Chapter 2 module's reusable first-class interfaces are
operation-bearing structures:
...
The same rule applies to Chapter 2's lattice exercises. `Lattice<s>` stores a
partial-order object together with callable `meet` and `join` operations. Its
remaining laws say that `meet(x,y)` is the greatest lower bound and
`join(x,y)` is the least upper bound:
...
The property records only the four distributive laws while callers continue
to use the original lattice object. A wrapper struct containing only the
parent lattice would add a second identity for the same operations without
adding mathematical data. Its downstream uses are the four standard
distribution projections and the equivalence of the meet-over-join and
join-over-meet formulations.
...
The order object already supplies reflexivity, transitivity, and
antisymmetry. The residual property says only that ring addition preserves
that order under left translation and products of nonnegative elements are
nonnegative. Its checked consumers are the two directions of
...
`additive_commutative_group_laws(s,add,zero,neg)` and
`ring_laws(s,add,zero,neg,mul,one)`. Thus a caller writes `ring.add`,
...
The `additive_commutative_group_laws`, `group_laws`, and `ring_laws`
propositions are candidate-data construction relations, not consumer
properties. Their names make that boundary explicit. Properties about an
already-formed system receive the corresponding object; old `is_*` aliases
are not retained.
...
residue. Its proof is an ordinary zero-factor contradiction over naturals; it
does not require a special product-positivity builtin rule.
...
one common natural cutoff. Constant, addition, scalar-multiplication, product,
and uniqueness proofs are checked. The reusable `converges_to_congr` theorem
transports convergence across pointwise-equal sequences and equal endpoints.
...
The construction-only `metric_space_laws(X,dist)` predicate occurs first in
Chapter 2 and validates the one-field `MetricSpace<X>` object. Chapter 2's
nonnegativity, zero-distance separation, and self-distance results consume
that object. Chapter 11 then uses `dist &MetricSpace<X>` throughout its balls,
convergence, compactness, completeness, continuity, and Baire interfaces.
Because a one-field structure is a named view of its sole function carrier,
the object remains directly callable as `dist(x,y)` while `dist.dist` is an
...
The elaboration-only settings `RealNormedSpaceSetting` and
`RealNormedSpacesSetting` collect the repeated ambient binders used by the
chapter's theorem interfaces. They expand to the same ordered public
parameters as the former explicit headers; props and templates keep explicit
parameters where those declarations define the mathematical interface rather
