# The Mechanics of Litex Proof draft

This directory is the canonical editable Litex module for *The Mechanics of
Litex Proof*. The ordered exports in [`litex.config`](litex.config) load the
preface, shared citation surface, and Chapters 0--10. The published tree at
`textbooks/The-Mechanics-of-Litex-Proof/` is a read-only snapshot until the
user explicitly requests publication.

Build and verify the complete draft with the release binary:

```text
cargo build --release
target/release/litex -compact -r scripts/textbooks_drafts/The-Mechanics-of-Litex-Proof
```

For an individual registered chapter, use:

```text
target/release/litex -compact -f scripts/textbooks_drafts/The-Mechanics-of-Litex-Proof/chapter09-sets.lit
```

As of 2026-08-01, Chapters 0--10 pass the complete release project gate. The
executable draft contains no `trust`, `axiom`, or `abstract_prop` statement.
...
target/release/litex -compact -session -before \
  scripts/textbooks_drafts/The-Mechanics-of-Litex-Proof/chapter10-relations.lit
```
...
`-r` gate. Do not edit the published snapshot as part of draft maintenance.
