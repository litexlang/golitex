...
```sh
RUST_MIN_STACK=8388608 target/release/litex -compact -r scripts/textbooks_drafts/Number-Theory-For-Beginners
```
...
The current draft contains 13 executable `trust` statements, no
...
```sh
target/release/litex -compact -f scripts/textbooks_drafts/Number-Theory-For-Beginners/section13.lit
```

The complete canonical draft also passes the release whole-book gate:
