...
use crate::simplify_polynomial::monomial::Monomial;
use super::collect_monomials::collect_ordered_monomials;
...
fn test_collect_ordered_monomials_number() {
    let obj = mk_num("1");
    let monomials = collect_ordered_monomials(&obj);
...
fn test_collect_ordered_monomials_add_like_terms() {
    ...
    let monomials = collect_ordered_monomials(&two_x_plus_three_x);
    assert_eq!(monomials.len(), 0);
}
...
fn test_collect_ordered_monomials_mul_number_and_var() {
    ...
    let monomials = collect_ordered_monomials(&two_x);
    assert_eq!(monomials.len(), 0);
}
...
fn test_collect_ordered_monomials_mul_two_vars() {
    ...
    let monomials = collect_ordered_monomials(&xy);
    assert_eq!(monomials.len(), 0);
}
...
fn test_collect_ordered_monomials_pow_number() {
    ...
    let monomials = collect_ordered_monomials(&two_sq);
    assert_eq!(monomials.len(), 1);
...
fn test_collect_ordered_monomials_pow_zero() {
    ...
    let monomials = collect_ordered_monomials(&x0);
    assert_eq!(monomials.len(), 1);
    assert_eq!(monomials[0].non_zero_scalar, "1");
    assert!(monomials[0].ordered_operands.is_none());
}
...
fn test_collect_ordered_monomials_pow_one() {
    ...
    let monomials = collect_ordered_monomials(&x1);
    assert_eq!(monomials.len(), 0);
}
...
fn test_collect_ordered_monomials_non_polynomial_returns_empty() {
    ...
    let monomials = collect_ordered_monomials(&list);
    assert_eq!(monomials.len(), 0);
}

#[test]
fn test_monomial_operands_equal() {
    let m_none = Monomial::new_and_check_scalar_is_not_zero("1".to_string(), None);
    let m_none2 = Monomial::new_and_check_scalar_is_not_zero    ("2".to_string(), None);
    assert!(m_none.is_some() && m_none2.is_some() && m_none.unwrap().operands_equal(&m_none2.unwrap()));

    let op_x = vec![(mk_var("x"), "x".to_string())];
    let m_x = Monomial::new_and_check_scalar_is_not_zero("1".to_string(), Some(op_x.clone()));
    let m_x2 = Monomial::new_and_check_scalar_is_not_zero("2".to_string(), Some(vec![(mk_var("x"), "x".to_string())]));
