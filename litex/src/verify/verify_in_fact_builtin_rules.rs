...
fn number_in_set_verified(num_value: &str, set_name: &str, reason: &str, line_file_index: Option<(usize, usize)>) -> StmtResult {
    StmtResult::FactVerifiedByBuiltinRules(
        ...
        match (&in_fact.element, &in_fact.set) {
            ...
            (Obj::Number(num), Obj::RPos(_)) => {
                if number_is_in_r_pos(num) {
                    Ok(number_in_set_verified(&num.value, R_POS, "number in R_pos", in_fact.line_file_index))
                } else {
            ...
            (Obj::Number(num), Obj::RNeg(_)) => {
                if number_is_in_r_neg(num) {
                    Ok(number_in_set_verified(&num.value, R_NEG, "number in R_neg", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::RNz(_)) => {
                if number_is_in_r_nz(num) {
                    Ok(number_in_set_verified(&num.value, R_NZ, "number in R_nz", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::QObj(_)) => Ok(number_in_set_verified(&num.value, Q, "number in Q", in_fact.line_file_index)),
            (Obj::Number(num), Obj::QPos(_)) => {
                if number_is_in_q_pos(num) {
                    Ok(number_in_set_verified(&num.value, Q_POS, "number in Q_pos", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::QNeg(_)) => {
                if number_is_in_q_neg(num) {
                    Ok(number_in_set_verified(&num.value, Q_NEG, "number in Q_neg", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::QNz(_)) => {
                if number_is_in_q_nz(num) {
                    Ok(number_in_set_verified(&num.value, Q_NZ, "number in Q_nz", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::ZObj(_)) => {
                if number_is_in_z(num) {
                    Ok(number_in_set_verified(&num.value, Z, "number in Z", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::ZPos(_)) => {
                if number_is_in_z_pos(num) {
                    Ok(number_in_set_verified(&num.value, Z_POS, "number in Z_pos", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::ZNeg(_)) => {
                if number_is_in_z_neg(num) {
                    Ok(number_in_set_verified(&num.value, Z_NEG, "number in Z_neg", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::ZNz(_)) => {
                if number_is_in_z_nz(num) {
                    Ok(number_in_set_verified(&num.value, Z_NZ, "number in Z_nz", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::NObj(_)) => {
                if number_is_in_n(num) {
                    Ok(number_in_set_verified(&num.value, N, "number in N", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
            (Obj::Number(num), Obj::NPosObj(_)) => {
                if number_is_in_n_pos(num) {
                    Ok(number_in_set_verified(&num.value, N_POS, "number in N_pos", in_fact.line_file_index))
                } else {
                    Ok(StmtResult::StmtUnknown(StmtUnknown::new()))
                }
            }
