...
impl<'a> Executor<'a> {
    ...
    pub fn verify_or_fact_with_known_or_facts(&mut self, or_fact: &OrFact) -> Result<NonErrStmtExecResult, VerifyError> {
        ...
        for environment in self.runtime_context.iter_environments_from_top() {
            let result = Self::verify_or_fact_with_known_or_facts_with_facts_in_environment(environment, or_fact, &all_objs_equal_to_each_arg)?;
            if result.is_true() {
                return Ok(result);
            }
        }

        let result = Self::verify_or_fact_with_known_or_facts_with_facts_in_environment(&self.runtime_context.builtin_environment, or_fact, &all_objs_equal_to_each_arg)?;
        if result.is_true() {
            return Ok(result);
        }
    ...
    pub fn verify_or_fact_with_known_or_facts_with_facts_in_environment(
        ...
        if let Some(known_or_facts) = environment.known_or_facts.get(&or_fact.key()) {
            for known_or_fact in known_or_facts.iter() {
                let result = Self::_verify_or_fact_the_same_type_and_return_matched_args(known_or_fact, or_fact)?;
                ...
                if let Some(matched_args) = result {
                    for (index, matched_arg) in matched_args.iter().enumerate() {
                        ...
                        if known_arg_string != matched_arg.1.to_string() && !all_objs_equal_to_each_arg[index].contains(&known_arg_string) {
