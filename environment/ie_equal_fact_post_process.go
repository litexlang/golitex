...
func (ie *InferenceEngine) equalFactPostProcess_tupleTuple(leftTuple *ast.FnObj, rightTuple *ast.FnObj) glob.GlobRet {
...
func (ie *InferenceEngine) equalFactPostProcess_tupleEquality(left ast.Obj, right ast.Obj) glob.GlobRet {
	...
	if leftIsTuple && rightIsTuple && ast.IsTupleFnObj(leftTuple) && ast.IsTupleFnObj(rightTuple) {
		...
		return ie.equalFactPostProcess_tupleTuple(leftTuple, rightTuple)
	} else if rightIsTuple && ast.IsTupleFnObj(rightTuple) {
		// 如果右边是 tuple，左边是对象: a = (1, 2, ..)
		return ie.equalFactPostProcess_tuple(left, right)
	} else if leftIsTuple && ast.IsTupleFnObj(leftTuple) {
		// 如果左边是 tuple，右边是对象: (1, 2, ..) = a
		return ie.equalFactPostProcess_tuple(right, left)
	}
